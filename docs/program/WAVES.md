...
**Status (2026-07-11, TODO #3 e614 re-cert):** **Release-gate green on tip for correctness gates** — full curated **19/19 green** on tip `e6145058bc214d8681e94dd449adff4620dfb281` (`artifacts/baseline/wave-e614-cert-merged/summary.json`, `CARGO_TARGET_DIR=/tmp/heddle-w-t3`; single `run-baseline.sh --suite curated` on detached clean worktree `/tmp/heddle-w-cert`, `dirty=0`). All oracles + fmt green. Clippy **`-D warnings` pass**, soft clippy **pass** (0 warnings), `cargo doc -p heddle-core --no-deps --locked` **pass**. Prior TODO N5 stamp on `96a422a8` (`todo-n5-cert-merged`), TODO R2 stamp on `6a09ecb7` (`todo-r2-cert-merged`), TODO #4 stamp on `a5b1dc68` (`todo4-curated-merged`), and Wave 8 stamp on `d3db0143` (`wave-next-merged`) remain historical greens; **superseded** for tip authority. Prior **n=5** core-loop perf sample retained (`PERF_BASELINE.md`) — **not** a Git win claim; multi-host perf still open. Remaining Wave 8 gaps: multi-host / platform matrix only (not a tip-correctness blocker).
---
