...
| L8 | Pack-without-index window between two durable publishes | objects `install_pack_files_streaming` | **Shipped A+ journal** — durable staging + intent (`pack_install_journal`); recover on `reload_packs`; Option D unpaired prune as backstop. See [`L8_PACK_INSTALL_JOURNAL.md`](L8_PACK_INSTALL_JOURNAL.md). Residual: in-memory `install_pack_files` not journaled; maintenance CLI surface optional. |
