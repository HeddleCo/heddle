...
| L8 | Pack-without-index window between two durable publishes | objects pack install | **Shipped A+** — staging + intent (streaming + in-memory); recover+TTL on `reload_packs`; **install/recover flock** + skip non-expired in-progress intents (concurrent reload-safe); Option D unpaired prune; maintenance/gc counters. See [`L8_PACK_INSTALL_JOURNAL.md`](L8_PACK_INSTALL_JOURNAL.md). Residual: hosted metrics product. |
