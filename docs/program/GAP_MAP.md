...
| L8 | Pack-without-index window between two durable publishes | objects pack install | **Shipped A+** — staging + intent for streaming *and* in-memory install; recover+TTL on `reload_packs`; Option D unpaired prune; maintenance/gc surfaces counters. See [`L8_PACK_INSTALL_JOURNAL.md`](L8_PACK_INSTALL_JOURNAL.md). Residual: hosted metrics product (not local library). |
