...
| L6 | Grandparent dirent durability on new object shards | objects `fs_atomic` (+ refs/oplog callers) | **Shipped on program tip** — `create_dir_all_durable` fsyncs newly created ancestors + deepest pre-existing parent; wired into object store atomic writes/publish/layout/pack/sidecars/agent-task/lock/registry/streaming buckets, private-dir create (`0o700`), **refs** init/lock/packed-refs parent/temp/remote paths + operation_index, **oplog** init + packed-oplog parent. Residual: tests-only bare `create_dir_all` fixtures; Windows dir fsync remains platform no-op. |
| L7 | `StreamingPackBuilder::finalize` staged pack/index durability | objects pack | **Shipped on program tip** — finalize fsyncs pack via `SyncData` (File) and index via `File::sync_all` + parent `sync_directory`. Publish still re-fsyncs at `publish_file_durable`. In-memory Cursor tests no-op. |
| L8 | Pack-without-index window between two durable publishes | objects pack install | **Shipped A+ hardened** — identifiers-only intent (paths reconstructed from trusted `packs_dir`); pair hash validation; quarantine unknown/malformed intents; flock + skip/TTL; stage outside lock; no Completed rewrite; maintenance/gc counters. See [`L8_PACK_INSTALL_JOURNAL.md`](L8_PACK_INSTALL_JOURNAL.md). Residual: full FS fault-injection matrix; per-pack locks/throughput measure; hosted metrics. |
