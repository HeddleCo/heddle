...
| L8 | Pack-without-index window between two durable publishes | objects `install_pack_files_streaming` | Window remains (pack then index durable renames). Unpaired packs still ignored by load (no silent corruption). **Design:** [`L8_PACK_INSTALL_JOURNAL.md`](L8_PACK_INSTALL_JOURNAL.md). **Foundation:** `list_unpaired_pack_files` / `prune_unpaired_pack_files` + `FsStore::prune_unpaired_packs` (Option D GC helpers). Full journal (Option A) still optional / not implemented. |
