...
The everyday workflow is `init` or `adopt`, then `status`, `diff`, `commit`, `start`, `ready`, `land`, and `push` or `pull`. `sync`, `resolve`, `continue`, and `abort` own workflow reconciliation. Manual `merge` and `rebase` implementations remain behind those workflow modules rather than competing as public verbs.
...
- Remove `switch`, `stash`, `clean`, `fetch`, `git-overlay`, `merge`, and `rebase` from the public command tree.
