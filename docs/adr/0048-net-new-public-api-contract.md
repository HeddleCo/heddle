...
| Subscription and billing reconciliation | Weft still implements a legacy billing-authorized entitlement write (`auth.rs:2108-2204`). Tapestry PR 164's Polar route still returns `fulfillment_not_ready` and names that missing entitlement write (`src/routes/api/webhook/polar/+server.ts:18-70`). The API manifest drops the route (`migration-manifest.json:142-146`) and exposes no replacement RPC. | Add the v1alpha1 write or land a reviewed reconciliation replacement, then port the Polar caller before removing the legacy route. Owner: `HeddleCo/api#1`, with Weft/Tapestry adapters. | **Blocked — API shape and Tapestry fulfillment missing** |
...
- [x] Merge the exact-head-reviewed matching Tapestry handle adapter
  ...
  `51c2c3e5`).
- [ ] Retain the legacy billing reconciliation route until its caller and
