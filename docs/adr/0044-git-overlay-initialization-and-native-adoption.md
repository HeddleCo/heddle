...
`heddle import local` is the explicit transition from Git Overlay into a Native Heddle Repository. Adoption moves source history into Heddle's native object model so native-only behavior can rely on Heddle source storage; it is not an alias for sidecar initialization or ordinary Git import.
...
- Help, status guidance, verification, and onboarding use `init` for normal Git Overlay setup and reserve `import local` for the explicit storage transition.
