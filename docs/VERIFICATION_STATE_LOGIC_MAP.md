...
- In Git-overlay mode, active Git reads and writes use the checkout's real
  ...
  `.heddle/git-projection/`; projection fidelity comes from reconstructable
  state and Raw Git Object Residuals.
...
| Capture/checkpoint | In Git Overlay, `capture` is one logical operation: save Heddle state, checkpoint Git, return one verification proof, and make one safe `undo` restore both when possible. Capture may not create a Heddle-only state if Git checkpoint preflight is blocked by remote divergence or import repair. |
...
| Generated artifact safety | `status`, `capture`, `ready`, and `land` must respect the ordered union of root `.gitignore` and `.heddleignore`, keep explicitly ignored changes from becoming fake work, and surface unignored generated/vendor/dist/build paths as ordinary dirty work or heavy-impact work when captured. Heddle must not install or assume default generated-noise patterns beyond `.heddle/`. Git-overlay captures above the existing safety limits require explicit force; every capture changing 500 or more paths also emits a non-blocking warning. |
