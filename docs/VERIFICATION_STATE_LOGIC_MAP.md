...
| Bridge import/reconcile JSON | Current `bridge git import` and `bridge git reconcile` post-operation JSON does not yet embed a `RepositoryVerificationState` proof. **Target:** both commands should return the same post-operation proof shape as other verification-aware mutating surfaces, so agents do not need a follow-up `verify` call to know the resulting state. |
...
- Remote divergence currently chooses between importing the upstream ref and
  ...
  path.
- `bridge git import` and `bridge git reconcile` post-operation JSON still need
  embedded `RepositoryVerificationState` proof payloads.
