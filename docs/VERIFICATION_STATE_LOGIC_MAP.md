...
| Generated artifact safety | `status`, `capture`, `commit`, `ready`, and `land` must respect the ordered union of root `.gitignore` and `.heddleignore`, keep explicitly ignored changes from becoming fake work, and surface unignored generated/vendor/dist/build paths as ordinary dirty work or heavy-impact work when captured. Heddle must not install or assume default generated-noise patterns beyond `.heddle/`. Git-overlay captures above the existing safety limits require explicit force; every capture changing 500 or more paths also emits a non-blocking warning. |
