...
`heddle land --threads` emits one batch envelope (for example,
`heddle land --threads alpha,beta --output json`):
...
```json
{
  ...
  "status": "landed",
  "action": "land",
  "message": "Landed 2 thread(s): alpha, beta",
  "threads": ["alpha", "beta"],
  "landed": ["alpha", "beta"],
  "peers": [
...
The command stops at the first blocked peer. A partial batch reports
`status: "partial"`, includes the successful prefix in `landed`, and names the
blocked peer in `stopped_at`.
...
## `heddle daemon serve --output json`
...
## `heddle daemon status --output json`
...
## `heddle daemon stop --output json`
...
```json
{
  ...
  "reachable": true,
  "token_kind": "agent",
  "scopes": ["repo:alice/api"],
  "operation_ceiling": ["GetState", "ListRefs", "Pull"],
  "expires_at": "2026-06-27T00:00:00Z",
  "ttl_seconds_remaining": 3600,
  "proof_key_available": true,
  "identity": {
    "subject": "did:key:z6Mk...",
    "actor_subject": "did:key:z6Mk...",
    "is_staff": false,
    "is_service_account": false,
    "is_biscuit": true,
    "session_id": "sess-123",
    "amr": ["webauthn"],
    "server_scope": "repo:alice/*",
    "credential_id": "cred-123",
    "device_id": "device-1",
    "agent_provider": null,
    "agent_model": null,
...
top-level `null`; the three pending-state fields are omitted rather than
serialized as `null`.
...
`heddle review next --output json` with no pending review emits:

```json
{"output_kind": "review_next", "next": null}
```
...
```json
{
  "command_contract_schema_coverage": {
    ...
    "advanced_scope_json_commands_total": 111,
    ...
    "json_mutating_commands_total": 93,
    "missing_mutating_schema_examples": [],
    "missing_schema_examples": [],
    "mutating_commands_total": 93,
    ...
    "status": "available",
    "summary": "201 command(s), 158 JSON command(s), 97 mutating command(s), 93 mutating JSON command(s); verified everyday/agent machine surface has 47 concrete schema-backed JSON command(s); advanced/internal/admin surfaces carry 44 accepted opaque schema(s) outside clean verification",
