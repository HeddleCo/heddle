...
`init`, `clone`, `status`, `workspace`, `start`, `capture`, `ready`,
`diff`, `merge`, `resolve`, `undo`, `log`, `show`, `thread`, `bridge`,
and `doctor`. Collaboration and annotation surfaces such as `review`,
`discuss`, and `context` stay behind `heddle help advanced` and their
topic pages for the moments you need them. First-time users see the
smallest surface that explains where they are, what is in flight, what
to do next, and how to recover.
