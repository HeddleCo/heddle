...
| 4 | **packfiles** `objects/packs/*.pack` | binary | `LMPK` magic + current `version = 4` + blake3 trailer; v3 remains readable; annotated tags use id-tag 2 | refuse unknown-newer with upgrade advice; unknown entry `ObjectType`/id-tag byte also refuses | `crates/pack/src/store/pack/mod.rs` (`pack_container_spec`, `ObjectType`), `shared.rs` (`verify_supported_container`) |
| 5 | **pack index** `*.idx` | binary | `LMI\0` + current `INDEX_VERSION = 4`; v3 remains readable; v4 reserves offset bit 62 for annotated-tag ids | refuse unknown-newer with upgrade advice | `pack_index.rs` |
