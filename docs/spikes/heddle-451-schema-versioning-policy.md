...
| 23 | **git-bridge mirror** + `git-bridge/bridge-mapping.json` | real git repo + JSON | NONE on the mapping | additive-tolerant JSON; the mirror itself is git-format (sley/git-owned). Mirror is slated for deletion (#568) | `crates/cli/src/bridge/git_mapping.rs:20-37,90,137` |
| 24 | **`heddle-submodule:` gitlink blob convention** | in-band magic prefix inside ordinary blob *content* | NONE | not a parse surface — export sniffs any `FileMode::Normal` blob whose content is `heddle-submodule:<oid>` and silently emits a gitlink. A user file with exactly that content is silently misinterpreted (low-probability, real class). Gitlinks have no first-class `EntryType`/`FileMode` | `crates/cli/src/bridge/git_import_tree.rs:22`, `git_export.rs:258-261,1097-1103` |
