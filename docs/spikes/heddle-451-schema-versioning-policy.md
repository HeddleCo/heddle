...
**Status:** accepted policy, refreshed for repository format v4 on 2026-08-13. **Motivated by:** the #449 brick — the
...
**Repository format v4 is the enforced compatibility gate.** `Repository::open`
refuses both newer formats and pre-v4 repositories before reading incompatible
...
| 4 | **packfiles** `objects/packs/*.pack` | binary | `LMPK` magic + current `version = 4` + blake3 trailer; annotated tags use id-tag 2 | accept only v4; refuse older versions with migration advice and unknown-newer versions with upgrade advice; unknown entry `ObjectType`/id-tag byte also refuses | `crates/pack/src/store/pack/mod.rs` (`pack_container_spec`, `ObjectType`), `shared.rs` (`verify_supported_container`) |
| 5 | **pack index** `*.idx` | binary | `LMI\0` + current `INDEX_VERSION = 4`; offset bit 62 marks annotated-tag ids | accept only v4; refuse older versions with migration advice and unknown-newer versions with upgrade advice | `pack_index.rs` |
...
| 19 | **repo config** `config.toml` | TOML | `[repository] version = 4`, enforced on open | refuse newer with `RepositoryFormatTooNew`; refuse older with `RepositoryFormatMigrationRequired`; neither path rewrites the config | `crates/repo/src/repo_config.rs`, `repository.rs` |
