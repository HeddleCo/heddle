...
| 24 | **legacy `heddle-submodule:` gitlink blob convention** | in-band magic prefix inside ordinary blob *content* | NONE | removed from runtime import/export and from the object model. Current trees use ADR-0041 first-class, format-aware Gitlink entries; older repositories require offline conversion. | `docs/adr/0041-first-class-gitlinks.md` |
| 25 | **reftable** `REFT01\0\0` | binary | magic, no version int | **removed spike — no production writer**; if revived it enters this policy with a migration from refs-text and a real version int | Historical format recorded in `docs/design/reftable-spike.md` |
...
- Any format change that older binaries could otherwise *silently* mishandle — or that
  changes unstamped-surface bytes — MUST bump the repo version as part of its clean
  cut, so older binaries refuse the whole repo instead of tripping mid-operation.
- Per-surface stamps (oplog, packs, …) still bump independently for changes that are
  locally detected; the repo version is the floor, not a
  replacement.
...
vs supported version, and the way out (upgrade, recreate, or re-adopt Git history).
...
- **Binary containers:** any layout change bumps the container version; the runtime
  admits exactly the paired version and refuses older or newer bytes without mutation.
...
### R5 — Clean cuts: explicit, offline, immutable
- The normal runtime has no schema ledger, compatibility reader, or rewrite-on-open.
- A format change bumps `[repository] version`; readers admit only the paired version
  and leave refused bytes unchanged.
- When persisted deployments need conversion, ship a bounded offline converter in
  the owning deployment plan. Do not put that machinery on every verb's open path.
- Content-addressed history is never silently re-identified. A conversion that changes
  identity must make the relationship explicit and be reviewed as a product cutover.
...
| **#575** first-class annotated-tag objects | rows 4, 7, 11 | New pack `ObjectType` → v3 refuses *any* pack containing a tag entry; loose layout addition is invisible/harmless; native tag bindings are new durable object data | Pack/index version 4 + repository version 4; v3 pack/index formats require repack/migration; sync/hosted transfer refuses unknown object discriminators |
...
3. **A golden-bytes format-lock test (the CI guard, recommendation for the issue’s
   ...
   fixture *alongside* the old one, and (c) records the offline cutover. This catches the
...
- **F2 (S):** typed too-new/too-old errors at all refusal sites
...
- **F5 (M):** implement ADR-0041 first-class Gitlink tree entries. Runtime export must stop sniffing
