...
same job. `reason` is gated behind the `ingest` feature, which
...
`reason` carries no canonical (`canonical_command: None`) and must be
...
- Driving the `--help` rewrite from the existing `canonical_command` /
  ...
  native command supersedes this bridge verb." (`reason` has no superseding
  canonical — `canonical_command: None` — and is documented as surface-only.)
...
- **Sub-issue A — overlay-aware `bridge git --help` (Options 1 + 4).**
  ...
  `reason` is surface-only (`canonical_command: None`) and must be handled
  explicitly rather than assumed to redirect. Size **S**. No JSON
  change.
