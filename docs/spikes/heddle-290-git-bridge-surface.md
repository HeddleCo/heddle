...
The agent/JSON surface is **already correct**: tiered to `advanced` and
redirecting each bridge verb to its native canonical. The defect is confined
to two **human-facing** surfaces that ignore the catalog:
...
directly fixes "users reach for the wrong verb," and it can be driven by the
metadata the catalog already holds (canonical_command per verb).
...
- Driving the `--help` rewrite from the existing `canonical_command` /
  ...
  keeps the human and machine surfaces from drifting apart — the catalog
  stays the single source of truth for "which native command supersedes this
  bridge verb."
