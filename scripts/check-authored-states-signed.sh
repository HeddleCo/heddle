...
#
# Scope: production command-contract code in `crates/cli-contract` plus the
# remaining command implementations in `crates/cli`. Test code builds unsigned
# fixtures on purpose, so we exempt:
...
SCAN_DIRS=(
  "$ROOT/crates/cli/src/cli/commands"
  "$ROOT/crates/cli-contract/src/cli/commands"
)
...
echo "OK: no authored-state signing-chokepoint bypasses under cli + cli-contract command trees"
