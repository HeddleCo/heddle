...
impl SignedOwnershipAcceptance {
    pub fn sign(value: &ThreadOwnershipClaim, acceptor: &impl Signer) -> Result<Self, Error> {
        if acceptor.public_key() != value.accepting_publisher {
...
impl SignedOwnershipClaim {
    pub fn sign(
        ...
        Ok(Self {
            canonical,
            local_signature: local.sign(&bytes)?,
            acceptance_signature: acceptor.sign(&bytes)?,
        })
    }
    pub fn verify(&self) -> Result<ThreadOwnershipClaim, Error> {
        let value = ThreadOwnershipClaim::decode(&self.canonical)?;
...
mod tests {
    use heddle_object_model::object::{
        CollaborationActor, ContentHash, thread_replication::SourceAuthor,
    };
    ...
    fn fixture() -> (ThreadOwnershipClaim, Ed25519Signer, Ed25519Signer) {
        ...
        let claim = ThreadOwnershipClaim {
            version: 1,
            thread: ContentHash::from_bytes([33; 32]),
    ...
    fn ownership_claim_browser_vector() {
        ...
        let proof = SignedOwnershipClaim::sign(&claim, &local, &acceptor)
            .expect("independent Rust fixture");
        assert_eq!(proof.verify().expect("verified vector"), claim);
        if std::env::var_os("HEDDLE_EXPORT_OWNERSHIP_VECTOR").is_some() {
            println!(
                "OWNERSHIP_VECTOR {{\"canonical\":{:?},\"acceptance_signature\":{:?}}}",
                proof.canonical, proof.acceptance_signature
            );
        }
    }
    #[test]
    fn ownership_claim_requires_both_exact_statement_signatures() {
        let (claim, local, acceptor) = fixture();
        let signed =
            SignedOwnershipClaim::sign(&claim, &local, &acceptor).expect("both signatures");
        assert_eq!(signed.verify().expect("verified"), claim);
        let mut missing_owner = signed.clone();
        missing_owner.local_signature = vec![0; 64];
        assert!(
            missing_owner.verify().is_err(),
            "local owner signature must be verified"
        );
        let mut missing_acceptor = signed.clone();
        missing_acceptor.acceptance_signature = vec![0; 64];
        assert!(
            missing_acceptor.verify().is_err(),
            "accepting account signature must be verified"
        );
        for mutate in 0..5 {
            ...
            match mutate {
                ...
                _ => {
                    let SourceAuthor::Account { actor, .. } = &mut changed.acceptance else {
                        panic!("account");
                    };
                    actor.principal_id = "00000000-0000-0000-0000-000000000027"
                        .parse()
                        .expect("UUID");
                }
            }
            let tampered = SignedOwnershipClaim {
                canonical: changed.encode().expect("valid different statement"),
                ..signed.clone()
            };
            assert!(
                tampered.verify().is_err(),
                "both signatures bind Thread/account/keys/cutoff"
            );
        }
    }
    #[test]
    fn local_integration_signature_binds_original_account_author() {
        use heddle_object_model::object::{
            Attribution, Principal, State, Tree, VisibilityTier,
            thread_replication::{
                ThreadOperation, ThreadOperationBody, local_integration::LocalIntegration,
            },
        };
        ...
        let state = State::new_snapshot(
            Tree::new().hash(),
            vec![],
            Attribution::human(Principal::new("source author", "")),
        );
        let receipt = LocalIntegration {
            ...
            author: claim.acceptance,
            source_thread: ContentHash::from_bytes([41; 32]),
            source_operation: ContentHash::from_bytes([42; 32]),
            source_revision: state.id(),
            target_thread: ContentHash::from_bytes([43; 32]),
            expected_target_frontier: Default::default(),
            result: state.encode_current_msgpack().expect("State").into(),
            result_visibility: VisibilityTier::Internal,
            initiating_request_proof: ContentHash::from_bytes([44; 32]),
            local_policy_version: ContentHash::from_bytes([45; 32]),
            executed_at_ms: 1,
        };
        let operation = ThreadOperation {
            ...
            body: ThreadOperationBody::LocalIntegration(
                receipt.encode().expect("authored integration"),
            ),
        };
        ...
        assert_eq!(
            signed
                ...
                .source_author()
                .expect("author"),
            Some(receipt.author.clone())
        );
        let mut changed = receipt;
        changed.author = SourceAuthor::LocalKey;
        let changed_operation = ThreadOperation {
            body: ThreadOperationBody::LocalIntegration(
                changed.encode().expect("different author"),
            ),
            ..operation
        };
        let substituted = SignedOperation {
            canonical: changed_operation.encode().expect("different source claim"),
            ..signed
        };
        assert!(
            substituted.verify().is_err(),
            "integration original author must be signed"
        );
    }
}
