...
use heddle_object_model::object::{
    CollaborationActor, ContentHash, StateId,
    thread_authority_admission::{OriginalAuthoritySubject, ThreadAuthorityAdmission},
    thread_replication::{
        GenesisOwner, SourceAuthor, ThreadGenesis, integration::TrustedHostedExecutor,
...
fn retained_claim_testimony_binds_subject_account_and_both_original_signatures() {
    ...
    let trust = TrustedHostedExecutor {
        spool: "00000000-0000-0000-0000-000000000094"
            .parse()
            .expect("Spool"),
        spool_genesis: ContentHash::from_bytes([97; 32]),
        executor: executor.public_key().try_into().expect("executor"),
    };
    let spool = trust.spool;
    let actor = CollaborationActor {
        principal_id: "00000000-0000-0000-0000-000000000095"
            .parse()
            .expect("account"),
    ...
    let claim = ThreadOwnershipClaim {
        ...
        prior_local_key: key,
        accepting_publisher: accepting.public_key().try_into().expect("acceptor"),
        acceptance: SourceAuthor::account(
            spool,
            actor.clone(),
            b"independently checked at original admission".to_vec(),
        )
        .expect("acceptance"),
        source_frontier: Default::default(),
    };
    let signed =
        SignedOwnershipClaim::sign(&claim, &local, &accepting).expect("both original signatures");
    ...
    changed.subject =
        OriginalAuthoritySubject::Operation(claim.id().expect("same digest, wrong kind"));
    assert!(
        SignedAuthorityAdmission::sign(&changed, &executor)
            .expect("signed wrong subject")
            .verify_claim(&signed, &genesis, &trust)
            .is_err(),
        "an operation receipt cannot authorize ownership"
    );
    let mut changed = statement.clone();
    changed.actor.agent_id = None;
    assert!(
        SignedAuthorityAdmission::sign(&changed, &executor)
            .expect("signed relabeling")
            .verify_claim(&signed, &genesis, &trust)
            .is_err(),
        "delegated acceptor identity cannot be relabeled"
    );
    for local_signature in [true, false] {
        let mut damaged = signed.clone();
        if local_signature {
            damaged.local_signature[0] ^= 1;
        } else {
            damaged.acceptance_signature[0] ^= 1;
        }
        assert!(
            receipt.verify_claim(&damaged, &genesis, &trust).is_err(),
            "historical receipt cannot replace an original signature"
        );
    }
    let mut wrong_trust = trust.clone();
    wrong_trust.executor = [98; 32];
    assert!(
        receipt
            .verify_claim(&signed, &genesis, &wrong_trust)
            .is_err(),
        "receipt issuer never establishes its own trust"
    );
}
