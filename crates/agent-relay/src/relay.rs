...
pub fn probe_current_process_harness(
    repo: &Repository,
...
/// Relay one harness hook payload into Heddle session state. `bridge` supplies
/// the CLI-owned capture/checkout implementations; see `crate::bridge`.
pub fn relay_harness_event(
    bridge: std::sync::Arc<dyn HarnessCliBridge>,
    repo: &Repository,
    harness: &str,
    event: &str,
    payload: &str,
...
fn init_harness_runtime(
    bridge: std::sync::Arc<dyn HarnessCliBridge>,
    repo: &Repository,
) -> Result<HarnessBridgeRuntime> {
...
pub struct HarnessBridgeRuntime {
    pub repo: Repository,
...
fn relay_codex(runtime: &mut HarnessBridgeRuntime, _event: &str, payload: &Value) -> Result<()> {
    ...
    runtime.update_progress(UpdateProgressParams {
        ...
        summary: value_string(payload, &["message"]),
        tool_name: value_string(payload, &["tool_name"]),
        detail: value_string(payload, &["tool_name"])
            .as_deref()
...
fn relay_opencode(runtime: &mut HarnessBridgeRuntime, event: &str, payload: &Value) -> Result<()> {
    ...
    runtime.update_progress(UpdateProgressParams {
        ...
        harness: Some("opencode".to_string()),
        status: Some(event.to_string()),
        tool_name: (event == "tool.execute.after").then(|| opencode_tool_name(payload)),
        detail: (event == "tool.execute.after")
...
fn record_timeline_tool_finished<E: HarnessTimelineExtractor>(
    ...
    let has_worktree_changes_before_capture =
        !collect_worktree_changes(&runtime.user_config, &runtime.repo)?.is_empty();
...
impl HarnessBridgeRuntime {
    ...
    pub fn relay(&mut self, harness: &str, event: &str, payload: &Value) -> Result<()> {
        match harness {
            ...
            "opencode" => relay_opencode(self, event, payload),
    ...
    fn ensure_harness_thread(
        ...
        let abs_path = self
            .bridge
            .prepare_worktree_target(&self.repo, &path, Some(name))?;
        self.bridge
...
fn compute_final_diff(
    ...
    let baseline_paths: BTreeSet<(String, String)> = worktree_baseline
        ...
        .collect();
    for (path, kind) in collect_worktree_changes(user_config, repo)? {
...
fn capture_worktree_change_snapshot(
    user_config: &UserConfig,
    repo: &Repository,
) -> Result<Vec<WorktreeChangeBaseline>> {
    Ok(collect_worktree_changes(user_config, repo)?
        .into_iter()
...
fn collect_worktree_changes(
    user_config: &UserConfig,
    repo: &Repository,
) -> Result<BTreeMap<String, DiffKind>> {
    let status_options = user_config.worktree_status_options(Some(repo.config()));
...
mod tests {
    ...
    /// Test stand-in for the CLI bridge: none of these tests reach capture or
    /// thread materialization; a stray call fails loudly instead of silently
    /// no-op'ing.
    struct MissingBridge;

    impl HarnessCliBridge for MissingBridge {
        fn capture_snapshot(
            &self,
            _repo: &Repository,
            ...
            anyhow::bail!("capture_snapshot is not available in relay unit tests")
        }
        ...
        fn prepare_worktree_target(
            ...
            _repo: &Repository,
            _path: &Path,
            _self_thread: Option<&str>,
        ) -> Result<PathBuf> {
            anyhow::bail!("prepare_worktree_target is not available in relay unit tests")
        }
        ...
        fn write_isolated_checkout(
            ...
            anyhow::bail!("write_isolated_checkout is not available in relay unit tests")
        }
    }

    fn test_bridge() -> std::sync::Arc<dyn HarnessCliBridge> {
        std::sync::Arc::new(MissingBridge)
    }
    ...
    fn close_session_captures_changed_paths_from_status_and_hints() {
        ...
        let mut runtime = HarnessBridgeRuntime::new(repo, config, test_bridge());
    ...
    fn same_native_actor_key_reuses_existing_actor_after_tentative_session_creation() {
        let (_temp, repo) = init_repo();
        let user_config = UserConfig::default();
        let runtime = HarnessBridgeRuntime::new(repo, user_config, test_bridge());
    ...
    fn relay_claude_user_prompt_submit_rotates_segment() {
        ...
        let mut runtime = HarnessBridgeRuntime::new(fresh, UserConfig::default(), test_bridge());
