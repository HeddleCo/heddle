...
pub struct RefEmitter<'a, R: RefBackend, S: LocalObjectStore> {
    refs: &'a R,
    store: &'a S,
    map: &'a ShaMap,
}

impl<'a, R: RefBackend, S: LocalObjectStore> RefEmitter<'a, R, S> {
...
fn change_is_ancestor<S: LocalObjectStore>(
    store: &S,
    ancestor: &ChangeId,
    descendant: &ChangeId,
) -> crate::Result<bool> {
