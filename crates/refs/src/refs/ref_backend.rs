...
pub trait RefBackend: CoreRefBackend<Error = HeddleError> {
    ...
    /// Whether this backend can durably commit a non-empty operation record
    /// batch at the [`commit_and_publish`](Self::commit_and_publish) seam.
    ///
    ...
    /// ref-carrying record batch (phase 4, typed `OpRecord`s) before publishing
