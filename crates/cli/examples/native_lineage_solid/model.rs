...
use objects::{
    ...
    store::{FsStore, ObjectStore, pack::ObjectType},
};
use serde::Serialize;
...
impl ObjectRef {
    ...
    pub fn load(&self, store: &FsStore) -> Result<Vec<u8>> {
        use objects::store::pack::PackObjectId;
        let id = match self {
            Self::State(id) => PackObjectId::StateId(*id),
            Self::Tree(hash) | Self::Blob(hash) => PackObjectId::Hash(*hash),
        };
        let (actual, bytes) = store
            .get_pack_object(&id)?
            .with_context(|| format!("missing native {} object {}", self.kind(), self.id()))?;
        let expected = match self {
            Self::State(_) => ObjectType::State,
            Self::Tree(_) => ObjectType::Tree,
            Self::Blob(_) => ObjectType::Blob,
        };
        if actual != expected {
            bail!(
                "native object {} resolved as {actual:?}, expected {expected:?}",
                self.id()
            );
        }
        self.validate(&bytes)?;
