...
pub use cli_args::{
    ...
    SessionStartArgs, ShellCommands, ShellKind, SnapshotArgs, StackArgs, StackCommands,
    StashCommands, StoreCommands, ThreadAbsorbArgs, ThreadCleanupArgs, ThreadCommands,
    ThreadDropArgs, ThreadListArgs,
    ThreadMoveArgs,
...
/// 4. `--json` CLI flag (deprecated; routes to JSON and emits a one-shot
///    stderr warning the first time it fires in a process)
///
...
/// Emit a deprecation notice for `--json` exactly once per process.
