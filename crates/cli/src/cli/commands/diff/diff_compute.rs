...
pub fn cmd_diff(
    ...
    if to.is_none()
        ...
        && let Some(probe) = build_plain_git_trust_probe(start)?
    {
...
fn render_plain_git_head_diff(
    ...
    let changes = probe
        ...
        .chain(probe.changes.added.iter().map(|path| FileChange {
            path: path.display().to_string(),
            kind: "added".to_string(),
            lines: None,
        }))
        .chain(probe.changes.deleted.iter().map(|path| FileChange {
            path: path.display().to_string(),
        ...
        .collect::<Vec<_>>();
    let output = DiffOutput {
        from_state: Some("HEAD".to_string()),
        to_state: None,
        changes,
