...
use objects::{
    object::{
        Blob, ContentHash, FileMode, State, StateAttachment, StateAttachmentBody, StateId,
        TreeEntry,
    },
    store::ObjectStore,
};
...
fn resume_manual_resolution_if_present(
    ...
    if current_state.state_id == pre_conflict_head || current_state.state_id == pending_commit {
        if !emit_rebase_progress(
            repo,
            cli,
            serde_json::json!({
    ...
    // `pre_conflict_head` to `current_state.state_id`, but unlike a
    ...
    let resolution_advance = match repo.head_ref()? {
        Head::Attached { thread } => OpRecord::FastForward {
            ...
            post_target_id: current_state.state_id,
        },
        Head::Detached { .. } => OpRecord::Goto {
            target: current_state.state_id,
            prev_head: Some(pre_conflict_head),
            head: current_state.state_id,
        },
    };
    ...
    if !emit_rebase_progress(
        ...
        serde_json::json!({
            ...
            "commit": pending_commit.short(),
            "resolved_as": current_state.state_id.short(),
        }),
    )? && cli.is_some()
    {
        println!(
...
fn apply_commit(
    ...
    copy_context_attachment(repo, commit_state, &new_state)?;
...
fn copy_context_attachment(
    repo: &Repository,
    source_state: &State,
    rebased_state: &State,
) -> Result<()> {
    let Some(source) =
        repo.latest_state_attachment(&source_state.state_id, StateAttachmentKind::Context)?
    else {
        return Ok(());
    };
    let StateAttachmentBody::Context(root) = source.body else {
        return Ok(());
    };
    let prior =
        repo.latest_state_attachment(&rebased_state.state_id, StateAttachmentKind::Context)?;
    ...
    repo.put_state_attachment(&StateAttachment {
        state_id: rebased_state.state_id,
        body: StateAttachmentBody::Context(root),
        attribution: rebased_state.attribution.clone(),
        created_at,
...
fn get_tree_for_state(repo: &Repository, state_id: &StateId) -> Result<ContentHash> {
...
mod tests {
    ...
    fn synthetic_goto_advance() -> OpRecord {
        let target = objects::object::StateId::from_bytes([7; 32]);
        OpRecord::Goto {
            target,
            prev_head: Some(objects::object::StateId::from_bytes([8; 32])),
            head: target,
        }
    }
