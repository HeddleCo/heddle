...
pub(crate) struct MergeOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trust: Option<RepositoryTrustState>,
}
...
fn merge_blocked_by_trust_output(
    ...
    MergeOutput {
        operator: OperatorCommandOutput {
            ...
            action: "merge".to_string(),
            message: format!(
                "Merge is up to date, but repository trust is blocked: {}",
                trust.summary
            ),
            blockers,
            warnings: Vec::new(),
        ...
        promotion_suggested: thread
            ...
            .unwrap_or(false),
        heavy_impact_paths: thread_heavy_paths(thread),
        semantic_result: preview_report.map(|report| report.semantic_result.clone()),
        conflict_count: 0,
...
fn emit_merge_output(cli: &Cli, output: MergeOutput) -> Result<()> {
    if should_output_json(cli, None) {
        ...
        if let Some(git_commit) = &output.git_commit {
            println!("  git commit: {}", style::dim(&git_commit.sha));
        }
