...
fn import_with_ref_filter(
    ...
    if repo.git_dir().join("shallow").is_file() {
        return Err(GitBridgeError::Git(format!(
            "shallow_clone: this Git repository is shallow; Heddle import needs the full ancestry. Run `git -C {} fetch --unshallow`, then rerun `heddle bridge git import{}`.",
        ...
        .map_err(git_err)?
    {
        ...
        if let Some(change_id) = bridge.mapping.get_heddle(oid) {
            ...
            if let Err(error) = bridge.heddle_repo.refs().create_marker(&name, &change_id) {
                match bridge.heddle_repo.refs().get_marker(&name) {
                    ...
                    Err(read_error) => {
                        return Err(GitBridgeError::InvalidMapping(format!(
                            "failed to create marker '{}' during git import: {}; also failed to re-read marker: {}",
                            name, error, read_error
                        )));
                    }
                }
            }
