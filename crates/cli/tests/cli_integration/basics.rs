...
fn test_cli_merge_preview_rejects_dirty_plain_git_repo_after_bootstrap() {
    ...
    assert!(
        err.contains("Refusing to merge")
            ...
            && err.contains("heddle capture -m")
            && err.contains("heddle stash push -m"),
        "merge preview should reject dirty current worktree with shared advice: {err}"
    );
}
...
fn test_cli_checkpoint_skips_when_git_index_is_locked() {
    ...
    assert!(
        envelope["error"]
            .as_str()
            .is_some_and(
                |error| (error.contains("locked") || error.contains("index"))
