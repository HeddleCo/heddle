...
fn extract_fixture(name: &str) -> (TempDir, std::path::PathBuf) {
    ...
    let extracted = git_substrate::GitRepo::open(&bare).expect("open extracted bare repo");
    let head = extracted
...
fn realworld_git_complex_fixture_round_trips_overlay_inventory_without_git_on_path() {
    ...
    let origin_repo = git_substrate::GitRepo::init_bare(&origin).expect("init synthetic real-world origin");
...
fn realworld_git_large_binary_blob_stress_without_git_on_path() {
    ...
    let origin = temp.path().join("origin.git");
    let work = temp.path().join("work");
    let origin_repo = git_substrate::GitRepo::init_bare(&origin).expect("init large origin");
...
fn realworld_git_multi_remote_divergent_main_resolves_origin_first() {
    ...
    let work = temp.path().join("work");
    let origin_repo = git_substrate::GitRepo::init_bare(&origin).expect("init origin");
    let upstream_repo = git_substrate::GitRepo::init_bare(&upstream).expect("init upstream");
...
fn realworld_git_annotated_tag_rename_round_trips() {
    ...
    let tag_a = git_create_annotated_tag(&origin_repo, "v0.1", &a, "v0.1 release\n");

    // Retarget the tag to B with a new message; substrate replaces the
    // previous tag object.
    let tag_b = git_create_annotated_tag(&origin_repo, "v0.1", &b, "v0.1 retargeted to B\n");
    assert_ne!(tag_a, tag_b, "retargeting should mint a new tag oid");
    ...
    let mirror_repo = git_substrate::GitRepo::open(&mirror).expect("open bridge mirror");
    let tag_oid = mirror_repo
        .read_ref_oid("refs/tags/v0.1")
    ...
    assert!(
        mirror_repo.has_object(&a).expect("check commit A"),
        "original commit A must remain reachable in the mirror"
    );
    assert!(
        mirror_repo.has_object(&b).expect("check commit B"),
...
fn marketing_moments_walkthrough_against_real_fixture() {
    ...
    let cloned = git_substrate::GitRepo::open(&work).expect("open cloned working tree");
    let head = cloned
        .head_commit_oid_or_none()
        .expect("head")
        .expect("head commit");
    git_set_reference(&cloned, "refs/heads/raw-side-branch", head);
    heddle_without_git(&["bridge", "import"], &work).unwrap();
