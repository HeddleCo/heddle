...
/// Fidelity-guard boundary (#593): a commit whose AUTHOR identity carries a raw
/// non-UTF8 byte must preserve that byte in `Principal.name` and reconstruct at
/// the original OID rather than routing through a verbatim residual.
#[test]
fn roundtrip_non_utf8_author_identity() {
    ...
    // `\xff` is never valid UTF-8; the committer is left clean to isolate the
    // author-identity case.
...
/// As [`roundtrip_non_utf8_author_identity`], for the distinct
/// `State.committer` principal.
#[test]
