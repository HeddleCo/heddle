...
fn test_merge_with_uncommitted_changes_fails() {
    ...
    assert!(
        error.contains("heddle capture") && error.contains("heddle stash push"),
    ...
    assert!(
        envelope["error"]
            ...
            .is_some_and(|error| error.contains("Unsafe:")
                ...
                && error.contains("Preserved:")
                && error.contains("Primary recovery:")
                && (error.contains("conflicting path") || error.contains("contested.txt"))),
        "refresh error on real conflict must name the conflicting path: {refresh_err}"
    );
    assert!(
        envelope["hint"]
            .as_str()
            .is_some_and(|hint| hint.contains("heddle merge beta --preview")
