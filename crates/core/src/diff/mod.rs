...
pub struct DiffOptions {
    ...
    /// Whether the report should include the top-level patch string when a
...
pub fn diff(ctx: &ExecutionContext, options: DiffOptions) -> Result<DiffReport> {
    let repo = ctx.require_repo().map_err(anyhow::Error::new)?;
    ...
    let semantic_diff_result = if options.semantic {
        if let Some(ref to_state) = to_state {
            ...
            Some(run_semantic_worktree_diff(
                repo,
                &from_hash,
                &status_options,
            )?)
    ...
    } else if let Some(ref to_state) = to_state {
        repo.diff_trees(&from_hash, &to_state.tree)?
    } else if git_overlay_head_worktree_diff {
        file_change_set_from_status(&repo.git_overlay_worktree_status()?.unwrap_or_default())
    } else {
        let tree = from_tree.clone().unwrap_or_default();
        file_change_set_from_status(
            &repo.compare_worktree_cached_with_options(&tree, &status_options)?,
        )
    };

    let patch_text_needed = options.include_patch_text;
    let want_hunks = patch_text_needed || !(options.name_only || options.stat);
    let file_changes = file_changes_from_change_set(
        repo,
        from_tree.as_ref(),
        to_tree.as_ref(),
        &changes,
    ...
    let semantic_changes = semantic_diff_result.map(|result| {
        result
            .changes
            .into_iter()
    ...
    let context_state = if options.show_context {
        if let Some(ref state) = to_state {
            Some(state.clone())
...
fn file_changes_from_change_set(
    ...
    options: &DiffOptions,
    want_hunks: bool,
    patch_text_needed: bool,
) -> Result<Vec<FileChange>> {
    let file_changes: Vec<FileChange> = if options.name_only && !patch_text_needed {
        changes
            .iter()
            .map(|change| {
        ...
        changes
            ...
            .map(|change| {
                ...
                let diff_result = if let Some(tree) = to_tree {
                    get_state_diff(repo, from_tree, tree, &change.path, &effective_kind)
                } else {
                    get_worktree_diff(repo, from_tree, &change.path, &effective_kind)
                };
                ...
                let (lines, line_counts) = if options.stat && !patch_text_needed {
                    ...
                    (
                        raw_lines.map(|lines| unified_hunks(lines, options.unified, &eol)),
                        None,
                    )
                };
...
pub fn diff_worktree_status(
    ...
    let want_hunks = options.include_patch_text && repo.is_some();
    let from_tree = match repo {
        Some(repo) => head_from_tree(repo)?,
        None => None,
    };
    let changes = file_changes_from_status(
        status,
        want_hunks,
        repo,
        from_tree.as_ref(),
    ...
    let changes = if detect_renames {
        match repo {
            Some(repo) => detect_clear_renames(
                repo,
                from_tree.as_ref(),
                None,
                changes,
...
fn file_change_set_from_status(status: &WorktreeStatus) -> FileChangeSet {
...
fn run_semantic_diff(
    repo: &Repository,
    from_tree_hash: &objects::object::ContentHash,
    to_tree_hash: &objects::object::ContentHash,
...
fn run_semantic_diff(
    _repo: &Repository,
    _from_tree_hash: &objects::object::ContentHash,
    _to_tree_hash: &objects::object::ContentHash,
) -> Result<SemanticDiffResult> {
    Err(anyhow!(HeddleError::recovery(
        RecoveryDetails::feature_unavailable("semantic diff", "semantic")
    )))
}

#[cfg(feature = "semantic")]
fn run_semantic_worktree_diff(
    repo: &Repository,
    from_tree_hash: &objects::object::ContentHash,
    ...
    let status = SemanticWorktreeStatus {
...
fn run_semantic_worktree_diff(
    _repo: &Repository,
    _from_tree_hash: &objects::object::ContentHash,
    _status_options: &repo::WorktreeStatusOptions,
) -> Result<SemanticDiffResult> {
    Err(anyhow!(HeddleError::recovery(
        RecoveryDetails::feature_unavailable("semantic diff", "semantic")
    )))
...
pub fn compute_tree_diff(
    ...
    let semantic_diff_result: Option<SemanticDiffResult> = if semantic {
        Some(run_semantic_diff(repo, &from_hash, &to_hash)?)
    } else {
        None
    };
...
/// context line.
pub fn trim_added_decorations_for_display(lines: &[LineDiff]) -> Vec<LineDiff> {
