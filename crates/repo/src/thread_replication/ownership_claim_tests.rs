...
use objects::{
    object::{
        ...
        thread_replication::{
            ...
            ThreadOperationBody, ownership_claim::ThreadOwnershipClaim,
        },
    },
    store::ObjectStore,
};
...
fn authority(root: &Ed25519Signer) -> crate::device_authority::DeviceAuthority {
    ...
    let signed =
        crate::sign_custodial_owner_root(root, &recovery, [9; 16], [5; 32]).expect("owner root");
    let binding =
...
fn acceptance(
    ...
    .expect("sealed acceptance");
    SourceAuthor::account(
        spool,
        CollaborationActor {
            principal_id: uuid::Uuid::from_bytes([9; 16]),
            agent_id: agent.then(|| "claim-agent".into()),
        },
        envelope,
    )
    .expect("account actor")
}
#[test]
fn explicit_claim_preserves_identity_cutoff_and_conflicts_fail_closed() {
    ...
    let local_record = crate::identity::LocalIdentity {
        public_key: hex::encode(local.public_key()),
        private_key_pem: local.to_pem().expect("local key material"),
    ...
    objects::fs_atomic::write_file_atomic_secret(
        ...
        toml::to_string(&local_record)
            .expect("local identity")
            .as_bytes(),
    )
    .expect("retain local key");
    let device = crate::identity::DeviceIdentity {
    ...
    objects::fs_atomic::write_file_atomic_secret(
        ...
        toml::to_string(&device)
            .expect("device identity")
            .as_bytes(),
    )
    .expect("enrolled different device key");
    let pair = biscuit_auth::KeyPair::from(
        &biscuit_auth::PrivateKey::from_bytes(&[71; 32], biscuit_auth::Algorithm::Ed25519)
            .expect("device root"),
    );
    ...
    crate::identity::source_author::publish(
        home.path(),
        &authority,
        &account.public_key().try_into().expect("key"),
        &account.public_key().try_into().expect("key"),
        &token,
        100,
    )
    .expect("retain enrollment proof");
    ...
    let genesis = ThreadGenesis {
        ...
        name: "unclaimed".into(),
        intent: "offline".into(),
        owner: GenesisOwner::LocalKey(local.public_key().try_into().expect("key")),
        creator: local.public_key().try_into().expect("key"),
        nonce: vec![],
    };
    ...
    let retained_signer = repository
        .native_thread_signer_at(&replica, home.path())
        .expect("local actions after enrollment");
    assert_eq!(
        retained_signer.public_key(),
        local.public_key(),
        "enrollment must not change unclaimed Thread signer"
    );
    assert_eq!(
        replica
            .source_author_for_at(
                &retained_signer.public_key().try_into().expect("key"),
                home.path()
            )
            .expect("source author"),
        SourceAuthor::LocalKey,
        "enrollment must not claim local Thread authorship"
    );
    let old = make("before claim");
    replica
        .receive(&old, repository.store(), |_| Ok(()))
        .expect("local source");
    let future_state = State::new_snapshot(
        tree,
        vec![base],
        Attribution::human(Principal::new("future account parent", "")),
    );
    let future_operation = ThreadOperation {
        version: 1,
        thread: replica.thread_id(),
        parents: Default::default(),
        publisher: account.public_key().try_into().expect("key"),
        body: ThreadOperationBody::Capture(AuthoredCapture {
            result: future_state
                .encode_current_msgpack()
                .expect("future State")
                .into(),
            author: crate::identity::source_author::load(
                home.path(),
                &account.public_key().try_into().expect("key"),
                spool,
            )
            .expect("account source proof"),
        }),
    };
    let future_parent = SignedOperation::sign(&future_operation, &account).expect("future parent");
    let delayed_state = State::new_snapshot(
        tree,
        vec![future_state.id()],
        Attribution::human(Principal::new("preclaim pending local work", "")),
    );
    let delayed_operation = ThreadOperation {
        version: 1,
        thread: replica.thread_id(),
        parents: [future_operation.id().expect("parent ID")].into(),
        publisher: genesis.creator,
        body: ThreadOperationBody::Capture(AuthoredCapture::local(
            delayed_state
                .encode_current_msgpack()
    ...
    assert_eq!(
        replica
            .receive(&delayed, repository.store(), |_| Ok(()))
            .expect("pending source admitted"),
        objects::object::thread_replication::Admission::Pending
    );
    let mut claim = ThreadOwnershipClaim {
        version: 1,
        thread: replica.thread_id(),
        prior_local_key: genesis.creator,
        accepting_publisher: account.public_key().try_into().expect("account key"),
        acceptance: acceptance(&authority, &account, spool, "ClaimThreadOwnership", true),
        source_frontier: [old.verify().expect("old").id().expect("id")].into(),
    };
    let denied_acceptance = acceptance(&authority, &account, spool, "PublishContent", true);
    let mut narrowed = claim.clone();
    narrowed.acceptance = denied_acceptance;
    let narrowed = SignedOwnershipClaim::sign(&narrowed, &local, &account).expect("narrowed claim");
    assert!(
        replica
            .claim_ownership(&narrowed, &authority, "acme/project", 100)
            .is_err(),
        "agent attenuation must constrain claim acceptance"
    );
    ...
    let command = crate::device_operations::Command {
        namespace: "owner/claim-agent",
        id: "00000000-0000-0000-0000-000000000077"
            .parse()
            .expect("command"),
        method: objects::object::thread_replication::ownership_claim::METHOD,
        request_hash: [7; 32],
    };
    assert_eq!(
        replica
            .claim_ownership_with_command(&signed, &authority, "acme/project", 100, &command, &[7])
            .expect("delegated account acceptance"),
        vec![7]
    );
    let id = claim.id().expect("claim ID");
    assert_eq!(
        replica
            .claim_ownership_with_command(&signed, &authority, "acme/project", 100, &command, &[8])
            .expect("exact request reuses original receipt"),
        vec![7]
    );
    assert_eq!(replica.thread_id(), genesis.id().expect("immutable ID"));
    assert_eq!(replica.genesis().expect("immutable genesis"), genesis);
    assert_eq!(
        replica.effective_owner().expect("account owner"),
        GenesisOwner::Account(uuid::Uuid::from_bytes([9; 16]))
    );
    assert!(
        replica
            .local_source_author_allowed(&old.verify().expect("old"))
            .expect("historical cutoff")
    );
    assert!(
        !replica
            .local_source_author_allowed(&make("after claim").verify().expect("new"))
            .expect("current owner"),
        "former local key cannot author outside signed cutoff"
    );
    assert_eq!(
        replica
            .receive(&future_parent, repository.store(), |_| Ok(()))
            .expect("authorized account parent after claim"),
        objects::object::thread_replication::Admission::Accepted
    );
    assert!(
        matches!(replica.operation(&delayed_operation.id().expect("delayed ID")).expect("delayed source").expect("known pending").1,
        objects::object::thread_replication::Admission::Rejected(ref reason) if reason.contains("cutoff")),
        "pending former-key source must not settle after signed cutoff"
    );
    assert!(
        replica
            .receive(
                &make("prepared before claim, delivered later"),
                repository.store(),
                |_| Ok(())
            )
            .expect_err("transaction must recheck claim")
            .to_string()
            .contains("cutoff"),
        "prepared local source must recheck effective ownership in commit transaction"
    );
    let claimed_signer = repository
        .native_thread_signer_at(&replica, home.path())
        .expect("claimed account device signer");
    assert_eq!(claimed_signer.public_key(), account.public_key());
    assert!(matches!(
        replica
            .source_author_for_at(
                &claimed_signer.public_key().try_into().expect("key"),
                home.path()
            )
            .expect("claimed source proof"),
        SourceAuthor::Account { .. }
    ));
    let generation = replica.generation().expect("generation");
    assert_eq!(
        replica
            .claim_ownership(&signed, &authority, "wrong/path", i64::MAX)
            .expect("retained exact proof no refresh"),
        id
    );
    ...
    claim.source_frontier = replica
        ...
        .collect();
    let conflicting =
        SignedOwnershipClaim::sign(&claim, &local, &account).expect("different valid claim");
    assert!(
        replica
            .claim_ownership(&conflicting, &authority, "acme/project", 100)
            .expect_err("competing explicit claim")
            .to_string()
            .contains("conflicting")
    );
    assert!(
        replica.effective_owner().is_err(),
        "conflicting claims must not select an arbitrary owner"
    );
    for retained in [&signed, &conflicting] {
        assert!(
            replica
                .claim_ownership(retained, &authority, "acme/project", 100)
                .expect_err("claim replay remains conflicted")
                .to_string()
                .contains("conflicting"),
            "retained claim replay must not assert resolved ownership"
        );
    }
    assert!(
        replica
            .claim_ownership_with_command(&signed, &authority, "acme/project", 100, &command, &[7])
            .expect_err("cached receipt remains conflicted")
            .to_string()
            .contains("conflicting"),
        "cached ownership receipt must not hide unresolved conflict"
    );
    assert!(
        !replica
            .local_source_author_allowed(&old.verify().expect("old"))
...
fn receipt_backfill(
    ...
    use objects::object::{
        ...
        thread_replication::integration::SPOOL_GENESIS_TRUST_FORMAT,
    };
    use prost::Message;
    ...
    let genesis =
        crate::sign_spool_owner_genesis(&owner, *spool.as_bytes()).expect("Spool genesis");
    ...
    let record = ThreadAuthorityAdmission {
        version: 3,
        basis: objects::object::original_boundary_acceptance::AdmissionBasis::OriginalAuthority,
        spool,
        spool_genesis: objects::object::ContentHash::compute_typed(
            SPOOL_GENESIS_TRUST_FORMAT,
            &genesis
                .genesis
                .as_ref()
                .expect("Spool statement")
                .encode_to_vec(),
        ),
        thread: replica.thread_id(),
        subject: OriginalAuthoritySubject::OwnershipClaim(statement.id().expect("claim ID")),
        actor,
        publisher: statement.accepting_publisher,
        authority_digest,
        executor: executor.public_key().try_into().expect("key"),
        admitted_at_ms: 2000,
    };
    let receipt =
        SignedAuthorityAdmission::sign(&record, &executor).expect("actual signed testimony");
