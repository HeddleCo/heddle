...
use objects::object::thread_replication::{
    SOURCE_AUTHORIZATION_METHOD, SourceAuthor, ThreadOperation,
};
...
impl ThreadReplica {
    pub fn verify_source_authority(
        ...
        if operation.thread != genesis.id()? {
            return Err(Error::Invalid("source author Thread differs".into()));
        }
        let author = operation
            .source_author()?
            .ok_or_else(|| Error::Invalid("source author gate requires authored source".into()))?;
        author.validate()?;
        match &author {
            ...
            SourceAuthor::Account {
                ...
                if spool.to_string() != genesis.spool {
                    return Err(Error::Invalid("source author Spool differs".into()));
                }
                ...
                if !self.audience_allows(
                    ...
                    return Err(Error::Invalid(
                        "original source author is outside current Thread audience".into(),
                    ));
                }
            }
        }
...
impl ThreadReplica {
    pub fn verify_local_source_owner(&self, operation: &ThreadOperation) -> Result<()> {
        if !self.local_source_author_allowed(operation)? {
            return Err(Error::Invalid(
                "local source author is outside the effective ownership cutoff".into(),
            ));
        }
        Ok(())
    }
