...
use objects::object::{
    ...
    thread_replication::{
        ...
        ownership_claim::{METHOD, ThreadOwnershipClaim},
    },
};
...
pub fn verify_claim_account_authority(
    ...
    authority
        .verify_publisher(&claim.prior_local_key)
        .map_err(|error| Error::Invalid(error.to_string()))?;
    authority
        .verify_publisher(&claim.accepting_publisher)
        .map_err(|error| Error::Invalid(error.to_string()))?;
...
impl ThreadReplica {
    ...
    pub(crate) fn source_author_for_at(
        &self,
        publisher: &[u8; 32],
        home: &std::path::Path,
    ) -> Result<SourceAuthor> {
        match self.effective_owner()? {
            ...
            GenesisOwner::LocalKey(_) => Err(Error::Invalid(
                "unclaimed Thread requires original local source signer".into(),
            )),
            GenesisOwner::Account(_) => {
                ...
                let author = crate::identity::source_author::load(home, publisher, spool)
                    .map_err(|error| Error::Invalid(error.to_string()))?;
                if matches!(author, SourceAuthor::LocalKey) {
                    return Err(Error::Invalid(
                        "account Thread requires original device account proof".into(),
                    ));
                }
                Ok(author)
            }
    ...
    pub(super) fn validate_source_owner_in(
        &self,
    ...
    pub fn ownership_claims_with_admission(
        &self,
    ) -> Result<
        Vec<(
        ...
        let rows = statement
            .query_map([self.thread.as_bytes()], |row| {
                Ok((
                    SignedOwnershipClaim {
                        canonical: row.get(0)?,
                        local_signature: row.get(1)?,
                        acceptance_signature: row.get(2)?,
                    },
 ...
 canonical,signature}),
    ...
    pub fn local_source_author_allowed(&self, operation: &ThreadOperation) -> Result<bool> {
        if operation.thread != self.thread
            || !matches!(operation.source_author()?, Some(SourceAuthor::LocalKey))
        {
            return Ok(false);
        }
        let genesis = self.genesis()?;
        if genesis.owner != GenesisOwner::LocalKey(operation.publisher) {
            return Ok(false);
        }
        let claims = self.ownership_claims()?;
        if claims.is_empty() {
 ...
 canonical,signature })),
    ...
    pub fn claim_ownership_with_admission(
        &self,
        signed: &SignedOwnershipClaim,
    ...
    fn claim_inner(
        ...
        if let Some(receipt) = admission {
            ...
            if let Some((authority, path, now)) = authority {
                ...
                return Err(Error::Invalid(
                    "claim requires original account authority or retained admission".into(),
                ));
            }
        }
        if command.is_some_and(|(command, response)| {
            command.namespace.is_empty()
                || command.namespace.len() > 1024
                || response.len() > 1024 * 1024
        }) {
            return Err(Error::Invalid(
                "ownership command receipt exceeds bounds".into(),
            ));
        }
        let mut connection = self.connect()?;
        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let count: i64 = transaction.query_row(
            "SELECT count(*) FROM thread_owner_claims WHERE thread=?1",
        ...
        if exact_frontier {
            ...
            if frontier != claim.source_frontier {
                return Err(Error::Invalid(
                    "ownership claim source frontier changed".into(),
                ));
            }
        } else {
            for head in &claim.source_frontier {
                ...
                if !accepted {
                    return Err(Error::Invalid(
                        "claim cutoff original source proof is missing".into(),
                    ));
                }
            }
        }
