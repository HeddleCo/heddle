...
use objects::{object::{ContentHash, StateId, TreeEntryTarget, thread_replication::ThreadOperation}, store::ObjectStore};
...
impl ThreadReplica {
    ...
    pub fn record_source_possession(&self, revision: StateId) -> Result<()> {
        ...
        tx.commit()?;
        if changed { self.notify_committed()?; }
        Ok(())
    }

    pub(super) fn record_source_possession_in(&self, tx: &Transaction<'_>, revision: StateId) -> Result<bool> {
        ...
        if !admitted { return Err(Error::Invalid("source possession requires an admitted source or exact genesis base".into())); }
        let inserted = tx.execute("INSERT OR IGNORE INTO thread_source_availability(thread,revision) VALUES(?1,?2)", params![self.thread.as_bytes(),revision.as_bytes()])?;
        if inserted != 0 {
    ...
    pub fn receive_prepared_source(&self, signed: &SignedOperation, store: &impl ObjectStore, authorize: impl FnOnce(&ThreadOperation)->Result<()>) -> Result<Admission> {
        let operation = signed.verify()?;
        if operation.thread != self.thread { return Err(Error::Invalid("wrong Thread".into())); }
        let state = operation.source_state()?.ok_or_else(|| Error::Invalid("prepared source operation required".into()))?;
        ...
        let mut connection = self.connect()?;
        let tx = connection.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        let admission = self.receive_in(&tx,signed,&operation,store,operation.local_integration()?.is_some(),None)?;
        if admission != Admission::Accepted { return Err(Error::Invalid("prepared source did not settle".into())); }
    ...
    pub(super) fn validate_local_source_possession(&self, store: &impl ObjectStore, revision: StateId) -> Result<()> {
        ...
        let state = store.get_state(&revision)?.ok_or_else(|| Error::Invalid("initial source State missing".into()))?;
        ...
        while let Some(hash) = trees.pop() {
            ...
            if seen.len() > 1_000_000 { return Err(Error::Invalid("initial source object budget exceeded".into())); }
            let tree = store.get_tree(&hash)?.ok_or_else(|| Error::Invalid("initial source tree missing".into()))?;
            for entry in tree.entries() {
                match entry.target() {
                    TreeEntryTarget::Tree { hash } => trees.push(*hash),
                    TreeEntryTarget::Blob { hash, .. } if seen.insert(*hash) => {
                        let blob = store.get_blob(hash)?.ok_or_else(|| Error::Invalid("initial source blob missing".into()))?;
                        bytes = bytes.checked_add(blob.size() as u64).ok_or_else(|| Error::Invalid("initial source byte overflow".into()))?;
                        if bytes > 512*1024*1024 || seen.len()>1_000_000 { return Err(Error::Invalid("initial source closure budget exceeded".into())); }
                    }
                    _ => {}
                }
            }
