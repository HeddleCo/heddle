...
use objects::{
    object::{
        ContentHash, State, VisibilityTier,
        thread_replication::{
            ThreadFacet, ThreadOperation, ThreadOperationBody, local_integration::LocalIntegration,
        },
    },
    store::ObjectStore,
};
...
impl DeviceRpc {
    pub(super) fn land_stack(
        &self,
        session: &Session,
        request: LandStackRequest,
    ) -> Result<Vec<u8>> {
        let Some(spool) = request.spool.as_ref() else {
            bail!("landing stack requires a Spool")
        };
        if spool.id != session.spool.id.to_string() {
            bail!("landing stack Spool does not match authority")
        }
        if request.landings.is_empty() || request.landings.len() > 64 {
            bail!("landing stack requires 1–64 members")
        }
        ...
        if !sources.is_disjoint(&targets) {
            bail!("landing stack source and target Threads must be disjoint")
        }
        let anchor = *targets
            .iter()
            .next()
            .context("landing stack target missing")?;
        let anchor_replica = ThreadReplica::open(&session.spool.heddle_dir, anchor)?;
        let prepared = anchor_replica.prepared_local_stack(
            &namespace,
        ...
        let (operations, response) = if let Some((canonical, response)) = prepared {
            for landing in &request.landings {
                if landing.expected_policy_version
                    != super::land::thread_policy_version(&repository)?.as_bytes()
                {
                    bail!("landing policy changed; observe landing again");
                }
                let source = ThreadReplica::open(
                    &session.spool.heddle_dir,
                    thread(session, landing.thread.as_ref())?,
                )?;
                let target = ThreadReplica::open(
                    &session.spool.heddle_dir,
                    thread(session, landing.target.as_ref())?,
                )?;
                session.authorize_thread(&repository, &source)?;
                session.authorize_thread(&repository, &target)?;
                source_visibility_floor(
                    &repository,
                    &source,
                    principal,
                    session.agent_id.as_deref(),
                    revision(session, landing.source.as_ref())?,
                )?
                ...
                source_visibility_floor(
                    ...
                    principal,
                    session.agent_id.as_deref(),
                    revision(session, landing.expected_target.as_ref())?,
                )?
                .context("target revision is unavailable to this caller")?;
            }
            let items: Vec<Vec<u8>> = rmp_serde::from_slice(&canonical)?;
            if items.len() != request.landings.len() {
                bail!("prepared stack length does not match request")
            }
            ...
            for (encoded, landing) in items.iter().zip(&request.landings) {
                ...
                let integration = operation
                    .local_integration()?
                    .context("prepared stack member is not a local integration")?;
                if operation.thread != thread(session, landing.target.as_ref())?
                    ...
                    || integration.local_policy_version.as_bytes().as_slice()
                        != landing.expected_policy_version
                {
            ...
            let mut initial_expected: BTreeMap<ContentHash, objects::object::StateId> =
                BTreeMap::new();
            ...
            for landing in &request.landings {
                ...
                if landing.expected_policy_version != policy.as_bytes() {
                    bail!("landing policy changed; observe landing again")
                }
                ...
                let cursor = if let Some(cursor) = cursors.get_mut(&target_id) {
                    if initial_expected.get(&target_id) != Some(&expected) {
                        bail!("stack target comparison changed within request")
                    }
                    cursor
                } else {
                    ...
                    let heads = if view.source_heads.is_empty() {
                        BTreeSet::from([target.genesis()?.base])
                    } else {
                        view.source_heads
                    };
                    if heads != BTreeSet::from([expected]) {
                        bail!(
                            "target changed or has unresolved source heads; observe landing again"
                        )
                    }
                    ...
                    let frontier = view
                        ...
                        .cloned()
                        .unwrap_or_default();
                    cursors.entry(target_id).or_insert(TargetCursor {
                        state: expected,
                        frontier,
                        floor: target_floor,
                        prepared: Vec::new(),
                ...
                let [source_operation] = source_originals.as_slice() else {
                    bail!("stack source must have one exact admitted original")
                };
                let tree = match verbs::merge::try_three_way_merge_between_tips(
                    &repository,
                    &cursor.state,
                    &source_state,
                    verbs::merge::ConflictLabels::DEFAULT,
                )? {
                    verbs::merge::ThreeWayMergeOutcome::Clean { tree } => {
                        repository.store().put_tree(&tree)?
                    }
                    verbs::merge::ThreeWayMergeOutcome::FastForward { target } => {
                        repository
                            .store()
                            .get_state(&target)?
                    ...
                    verbs::merge::ThreeWayMergeOutcome::AlreadyIntegrated { .. } => {
                        repository
                            .store()
                            .get_state(&cursor.state)?
                            .context("target state missing")?
                            .tree
                    }
                    verbs::merge::ThreeWayMergeOutcome::Conflicted { paths, .. } => bail!(
                ...
                let integration = LocalIntegration {
                    ...
                    expected_target_frontier: cursor.frontier.clone(),
                    result: target.prepare_integration_with_prepared(
                        &repository,
                        &state,
                        source_id,
                        *source_operation,
            ...
            .encode_to_vec();
            let canonical = rmp_serde::to_vec(
                &operations
                    .iter()
                    .map(ThreadOperation::encode)
                    .collect::<Result<Vec<_>, _>>()?,
            )?;
            let (canonical, response) = anchor_replica.prepare_local_stack(
                &namespace,
                &request.client_operation_id,
                &request_hash,
                &canonical,
            ...
            let operations = bytes
                .iter()
                .map(|bytes| ThreadOperation::decode(bytes))
                .collect::<Result<Vec<_>, _>>()?;
            (operations, response)
        };
        ...
        session.check_current(&self.home)?;
        anchor_replica.receive_local_stack_cas_command(
            &signed,
            repository.store(),
            &command,
            &response,
