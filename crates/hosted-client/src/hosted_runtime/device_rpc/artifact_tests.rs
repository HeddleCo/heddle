...
pub(super) async fn roundtrip(
    ...
    let policy_version = runs
        .policy(&spool.to_string())
    ...
    .expect("opt in");
    let store =
        repo::device_artifacts::ArtifactStore::open(repository.heddle_dir()).expect("catalog");
    let retained = store
        .retain(
            &run,
            "report",
    ...
    let mut stream = remote
        .api
        .observe::<thread_api::rpc::ContentServiceReadArtifact>(&request)
        .await
    ...
    assert!(
        matches!(stream.next().await.expect("completion").expect("event").payload, Some(artifact_event::Payload::Complete(status)) if status.coverage == Coverage::Complete as i32)
    );
    ...
    for denied in [
        ...
        let mut stream = remote
            .api
            .observe::<thread_api::rpc::ContentServiceReadArtifact>(&denied)
            .await
            .expect("typed stream opens");
        assert!(
            matches!(stream.next().await, Err(api::v2::client::ClientError::Transport(thread_api::transport::Error::Remote(failure))) if failure.code == CallFailureCode::FailedPrecondition as i32),
            "invalid artifact request must fail before a chunk"
        );
    }
    let policy_version = runs
        .policy(&spool.to_string())
    ...
    assert!(
        matches!(stream.next().await, Err(api::v2::client::ClientError::Transport(thread_api::transport::Error::Remote(failure))) if failure.code == CallFailureCode::FailedPrecondition as i32),
        "current policy must deny previously retained data"
    );
}
