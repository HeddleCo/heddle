...
impl DeviceRpc {
    pub(super) fn claim_thread_ownership(&self, session: &Session, body: &[u8]) -> Result<Vec<u8>> {
        ...
        let accepting_account = match &proof {
            ...
            accepting_account == uuid::Uuid::parse_str(&session.principal)?,
            "claim acceptance belongs to another authenticated account"
        );
        ...
        let complete = match proof {
            ...
            ClaimProof::Acceptance(acceptance) => {
                ...
                if let Some(retained) = replica.ownership_claims()?.into_iter().find(|proof| {
                    proof.canonical == acceptance.canonical
                        && proof.acceptance_signature == acceptance.signature
                }) {
                    retained
                } else {
                    verify_claim_account_authority(
                        &statement,
                        &replica.genesis()?,
                        &authority,
                    ...
                    let frontier = replica
                        .frontier_page(ThreadFacet::Source, None, 129)?
                        .into_iter()
                        .collect();
                    ensure!(
                        statement.source_frontier == frontier,
                        "ownership claim source frontier changed"
                    );
                    ensure!(
                        replica.effective_owner()?
                            == objects::object::thread_replication::GenesisOwner::LocalKey(
                                statement.prior_local_key
                            ),
        ...
        let response = ClaimThreadOwnershipResponse {
            ...
            owner: Some(PrincipalRef {
                id: statement.account()?.to_string(),
            }),
            claim_id: claim_id.as_bytes().to_vec(),
            claim: Some(claim_codec::encode(&complete)?),
        }
        .encode_to_vec();
        session.check_current(&self.home)?;
        session.authorize_thread(&repository, &replica)?;
        Ok(replica.claim_ownership_with_command(
            &complete,
            &authority,
            &session.spool.capability_path,
            now,
...
pub(super) fn ownership_view(replica: &ThreadReplica) -> Result<ThreadOwnership> {
    ...
    if claims.len() > 1 {
        return Ok(ThreadOwnership {
            owner: Some(thread_ownership::Owner::Conflict(ThreadOwnershipConflict {
                claim_ids: claims
                    .iter()
                    .map(|claim| Ok(claim.verify()?.id()?.as_bytes().to_vec()))
    ...
    let owner = match replica.effective_owner()? {
        objects::object::thread_replication::GenesisOwner::LocalKey(key) => {
            thread_ownership::Owner::LocalKey(key.to_vec())
        }
        objects::object::thread_replication::GenesisOwner::Account(account) => {
            thread_ownership::Owner::Account(PrincipalRef {
    ...
    Ok(ThreadOwnership {
        ...
        claim_id: claims
            .first()
            .map(|claim| -> Result<Vec<u8>> { Ok(claim.verify()?.id()?.as_bytes().to_vec()) })
            .transpose()?
            .unwrap_or_default(),
    })
}
...
impl DeviceRpc {
    ...
    pub(super) fn thread_conflict_status(
        &self,
        session: &Session,
        ...
        let objects::object::thread_replication::GenesisOwner::LocalKey(key) = genesis.owner else {
            anyhow::bail!("conflicted Thread has no original local owner");
        };
        let repository = repo::Repository::open(&session.spool.root)?;
        ensure!(
            repository.holds_native_owner_key(&key)?,
            "ownership conflict status requires retained original local owner key"
        );
