...
use crate::{
    contract::*,
    fetch::{Error, ValidatedSourceArtifacts},
};
...
pub fn validate_source_artifacts(
    ...
    let thread = opening
        .thread
        .as_ref()
        .ok_or(Error::Invalid("publication Thread absent"))?;
    let revision = opening
        .revision
        .as_ref()
        .ok_or(Error::Invalid("publication revision absent"))?;
    ...
    for (extent, name) in opening.packs.iter().zip(["source.pack", "source.idx"]) {
        let address = extent
            .pack
            .as_ref()
            .ok_or(Error::Invalid("artifact address absent"))?;
        total = total
        ...
        if file.metadata()?.len() != extent.length {
            return Err(Error::Invalid("uploaded artifact length differs"));
        }
    ...
    let selected = thread
        .id
        .as_ref()
        .ok_or(Error::Invalid("Thread identity absent"))?;
    ...
    for wrapper in originals.geneses {
        ...
        if candidate.id().map_err(preparation)?.as_bytes().as_slice() == selected.value {
            if genesis.replace(wrapper).is_some() {
                return Err(Error::Invalid("duplicate selected genesis"));
            }
        } else {
    ...
    crate::fetch::validate_artifacts(
        ...
        thread,
        revision,
        &genesis.ok_or(Error::Invalid("original selected genesis absent"))?,
        operations,
        dependencies,
        receipts,
    )
