...
fn attached_endpoint_rejects_different_credential_account() {
    ...
    let token = Biscuit::builder()
        .fact("user(\"other-account\")")
        .expect("user")
        .fact("subject_user_uuid(\"00000000-0000-0000-0000-000000000022\")")
        .expect("different account")
        .fact("session(\"other-account-session\")")
        .expect("session")
        .fact(format!("device_pop_key(\"{}\")", hex::encode(subject.public_key())).as_str())
        .expect("subject")
        .build(&root)
    ...
    .expect("correct signatures");
    let error = root_attachment::verify(
        &attachment,
        &token,
        &[root.public()],
        ACCOUNT,
