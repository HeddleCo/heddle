...
fn fixture() -> (SignedMintRootAttachment, VerifiedOwnerState) {
    ...
    let root = signed_root(OWNER_UUID, &owner, &[
    ...
    let certificate = SignedPasskeyAuthority {
        owner_signature: Some(owner.sign_digest(&passkey_authority_signing_digest(&authority).expect("certificate digest"))),
    ...
    (SignedMintRootAttachment {
        ...
        passkey_delegation: Some(PasskeyMintDelegation {
            authority: Some(certificate), client_data_json, authenticator_data, signature,
        }),
    }, state)
}

fn verify(value: &SignedMintRootAttachment, state: &VerifiedOwnerState) -> Result<()> {
    verify_mint_root_attachment(value, state, &OWNER_UUID, &TestKey::new(83).wire().public_key, NOW + 1)
}

#[test]
fn owner_authorized_passkey_admits_temporary_mint_without_server_or_existing_device() {
    let (value, state) = fixture();
    verify(&value, &state).expect("owner certificate plus fresh passkey assertion authorizes temporary mint");
}
...
fn passkey_assertion_binds_exact_mint_origin_rp_and_user_verification() {
    ...
    for mutate in [
        |value: &mut SignedMintRootAttachment| value.attachment.as_mut().expect("attachment").nonce[0] ^= 1,
        |value: &mut SignedMintRootAttachment| value.passkey_delegation.as_mut().expect("proof").authenticator_data[0] ^= 1,
        |value: &mut SignedMintRootAttachment| value.passkey_delegation.as_mut().expect("proof").authenticator_data[32] = 0x01,
        |value: &mut SignedMintRootAttachment| value.passkey_delegation.as_mut().expect("proof").signature[0] ^= 1,
        |value: &mut SignedMintRootAttachment| {
            let proof = value.passkey_delegation.as_mut().expect("proof");
            let mut client: serde_json::Value = serde_json::from_slice(&proof.client_data_json).expect("client");
        ...
        mutate(&mut changed);
        assert!(verify(&changed, &state).is_err(), "mutated authority must be rejected");
    }
}
...
fn passkey_authority_cannot_extend_lifetime_or_select_another_owner() {
    ...
    verify(&valid, &state).expect("positive control");
    let mut changed = valid.clone();
    changed.attachment.as_mut().expect("attachment").expires_at_unix_seconds = NOW + 43_201;
    assert!(verify(&changed, &state).is_err());
    let mut changed = valid.clone();
    let certificate = changed.passkey_delegation.as_mut().expect("proof").authority.as_mut().expect("certificate");
    certificate.authority.as_mut().expect("authority").account_uuid = [1; 16].to_vec();
    assert!(verify(&changed, &state).is_err());
    let mut changed = valid;
    changed.owner_signature = Some(TestKey::new(81).sign_digest(&mint_root_signing_digest(changed.attachment.as_ref().expect("attachment")).expect("digest")));
    assert!(verify(&changed, &state).is_err(), "ambiguous authority forms must be rejected");
}
